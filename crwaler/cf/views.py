...
def cf_home(request):
    if request.method == "POST":
        url = 'https://codeforces.com/api/user.info?handles=' + str(request.POST['name'])
        response = requests.get(url)
        soup2 = bs4.BeautifulSoup(response.text, "html.parser")
        l2 = json.loads(str(soup2))
        if l2['status'] != 'OK':
            return render(request, 'cf/cf_home.html', {'flag': 1})
        else:
            return HttpResponseRedirect('/cf/' + str(request.POST['name'] + '/'))
    return render(request, 'cf/cf_home.html', {'flag': 0})
...
def analysis(request, name):
    ...
    attempts = len(l['result'])
    attempted_questions = set([])  # attempted questions
    solved_questions = set([])
    unsolved_questions = set([])  # unsolved questions
    ...
    for item in l['result']:
        ...
        if item['verdict'] != "OK":
            unsolved_questions.add(we)

        languages.append(item['programmingLanguage'])
        attempted_questions.add(asd)
        if item['verdict'] == "OK":
    ...
    for i in solved_questions:
        if i in unsolved_questions:
            unsolved_questions.remove(i)
        else:
            first_time_ac += 1
    unsolved_problems = list()
    for i in unsolved_questions:
        pr_name = str(i[42:])
        my_pr_name = ""
        for j in pr_name:
            if j != '/':
    ...
    tried = len(attempted_questions)
    accepted = len(attempted_questions) - len(unsolved_questions)
    ...
    avg_sub = (100 * attempts // accepted) / 100

    return render(request, 'cf/cf_analysis.html',
                  {'name': name, 'c': col, 'contest_count': contest_count, 'sub': sub, 'sol_num': 50,
                   'attempts': attempts,
                   'ac': ac, 'other': other, 'tried': tried, 'chart1': dump, 'chart2': dump2, 'worst_rank': worst_rank,
